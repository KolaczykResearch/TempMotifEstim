...
public:
	...
	vector<double> ExactCountMotifs(int delta, vector<long>&ids) {
		...
		vector<double> rr={-1,-1};
		return rr;
		// return -1;
    }

  vector<double> Cmotifs(int delta, int p,vector<TEdge>&edges,vector<TEdge>&fs,int S_order[][4])
	{
	  unordered_map<int,double> eta_fs_map;
	  /*// initialize local count eta(e_i) for each selected e_i as zero
	  for(int i=0;i<fs.size();i++)
	  {
	    eta_fs_map[fs[i].id] = 0;
	    // cout<<fs[i].id<<"\n"<<endl;
	  }
	  */
		double sum=0.0;
    		...
    		while (flag==0){
    			...
    			if (eG < (int) edges.size()) {
    				if (eStack.size()== int(edgesM_.size() - 1)) {
    					...
    					if (p == -1){
    					  res += 1;
    					  if (eta_fs_map.count(edges[eStack[0]].id)>0){
    					    // is an element
    					    eta_fs_map[edges[eStack[0]].id] += 1;
    					  }else{
    					    eta_fs_map[edges[eStack[0]].id] = 1;
    					  }
    					  // eStack[0] is the edge index for the i-th matching edge in the motif instance with l edge
    					  // cout<<"\n res="<<res<<" "<<edges[eStack[0]].id<<" "<<edges[eStack[1]].id<<" "<<edges[eG].id<<endl;
    					} 
    					else res += 1.0/p;
		...
		// calculate varaince with the local counts eta_fs_map
		// double sum2=0.0;
		for(std::unordered_map<int,double>::iterator itr=eta_fs_map.begin();itr!=eta_fs_map.end();itr++){
		  sum_squared = sum_squared + pow(itr->second, 2);
		  // sum2 = sum2 + (itr->second);
		  // cout<<"itr= "<<pow(itr->second, 2);
		}
		// cout<<"sum is "<<sum<<"\n";
		// cout<<"sum2 is "<<sum2<<"\n";
		vector<double> results;
		results.push_back(sum);
		results.push_back(sum_squared);
		return results;
		// return sum;
	}
